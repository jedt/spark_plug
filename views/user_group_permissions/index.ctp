...
|
<?php echo $html->link(__('New User Group Permission', true), array('action'=>'add')); ?>

<h2>User Group Permissions</h2>
<div class="table_wrap browse">
<table class="full" cellspacing="1" cellpadding="4" border="0"
	...
	<?php
	// adds the filter if the component is present
	if ($filterEnabled){
		echo $filter->form('UserGroupPermission', array(null,null,'plugin','controller','action','allow'));
	}
	?>
	<?php
	$i = 0;
	...
	?>
	<tr <?php echo $class;?>>
		<td><?php echo $permission['UserGroupPermission']['id']; ?></td>
		<td><?php echo $permission['UserGroup']['name']; ?></td>
